...
test('live smoke proves anonymous and cross-user token access is denied', async () => {
  ...
  const fetchImpl = async (input, options = {}) => {
    ...
    if (requestUrl.pathname === '/auth/v1/admin/users' && options.method === 'POST') {
      ...
      const id = body.email.includes('-victim-') ? victimId : attackerId
      authUsers.set(id, body.email)
      return response(200, { id })
    }
    if (requestUrl.pathname === '/auth/v1/admin/users') {
      return response(200, {
        users: [...authUsers].map(([id, email]) => ({ id, email })),
      })
    }
    ...
    if (requestUrl.pathname.startsWith('/auth/v1/admin/users/')) {
      ...
      return response(authUsers.has(id) ? 200 : 404)
    }
...
test('cleanup recovers an auth user when create succeeds but its response loses the id', async () => {
  const projectRef = 'mxnmubakfzqoosgsqmhh'
  const url = `https://${projectRef}.supabase.co`
  const fixtureId = '33333333-3333-4333-8333-333333333333'
  const victimId = '11111111-1111-4111-8111-111111111111'
  const victimEmail = `codex-token-rls-victim-${fixtureId}@example.invalid`
  const encode = (value) => Buffer.from(JSON.stringify(value)).toString('base64url')
  const jwt = (claims) => `${encode({ alg: 'HS256' })}.${encode(claims)}.signature`
  const anon = jwt({ role: 'anon', ref: projectRef, iss: 'supabase', exp: 4_102_444_800 })
  const service = jwt({ role: 'service_role', ref: projectRef, iss: 'supabase', exp: 4_102_444_800 })
  ...
  const fetchImpl = async (input, options = {}) => {
    ...
    if (requestUrl.pathname === `/auth/v1/admin/users/${victimId}`) {
      if (options.method === 'DELETE') {
        users.delete(victimId)
        return response(204)
      }
      return response(users.has(victimId) ? 200 : 404)
    }
    if (requestUrl.pathname === '/rest/v1/telegram_link_tokens') {
      return response(options.method === 'DELETE' ? 204 : 200, options.method ? undefined : [])
    }
    if (requestUrl.pathname === '/rest/v1/profiles') return response(200, [])
    throw new Error(`unexpected request: ${options.method ?? 'GET'} ${requestUrl.pathname}`)
  }
  ...
  assert.equal(result.assertions.find((item) => item.id === 'smoke-harness-execution')?.status, 'failed')
  assert.equal(result.assertions.find((item) => item.id === 'synthetic-users-deleted')?.status, 'passed')
  assert.equal(result.assertions.find((item) => item.id === 'synthetic-data-deleted')?.status, 'passed')
  assert.equal(users.size, 0)
  assert.doesNotMatch(JSON.stringify(result), new RegExp(`${victimId}|${victimEmail}|${fixtureId}`))
})
