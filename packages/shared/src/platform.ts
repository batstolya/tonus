// Platform hooks for cross-client code (mobile monorepo Phase 0b).
// Mirrors env.ts: platform wiring (web: apps/web/src/lib/platform.web.ts,
// tests: vitest.env-setup.ts, mobile: apps/mobile/src/platform.native.ts) must call
// initPlatform() before lib code runs. Web Storage and browser locale APIs
// must not be touched anywhere else — React Native has neither.
