...
- **Статус:** воркстримы A, C, D ЗАВЕРШЕНЫ; B — первый заход готов (расширение опционально).

## Сессия 3 (2026-07-13, вечер) — воркстримы D и C добиты

| PR | Суть |
|---|---|
| [#56](https://github.com/batstolya/tonus/pull/56) | **Воркстрим D:** зеркало `scores.ts` удалено, единый источник = `_shared/scores.ts` (`src/lib/scores.ts` = фасад). Оказалось дёшево, не кодоген: тулчейн уже импортит через границу src↔supabase. ingest-health передеплоен. |
| [#57](https://github.com/batstolya/tonus/pull/57) | **Воркстрим C (данные):** `translations.ts` 1063 → 8 доменных файлов + barrel. Строки резались дословно скриптом, deep-equal тест (915 ключей) перед удалением. |
| [#58](https://github.com/batstolya/tonus/pull/58) | **Воркстрим C (поведение):** `SettingsScreen.tsx` 854 → 176. God-компонент → `sections/*.tsx` под защитой характеризационного теста (мок Supabase, сквозной архив ДО/ПОСЛЕ). Крупнейший осколок 198. |

**Итог: ни одного продуктового файла > ~270 строк (кроме сгенерированного
`database.types.ts`). Формулы scores — единый источник. Все цели воркстримов
A/C/D достигнуты.**

Осталось (мелочь, под храповиками): lint 21 (19 react-hooks поведенческих +
no-namespace + react-refresh — рискованные), deno 16 (14 в `_shared/*.test.ts`).
Плюс опциональное расширение B (тесты остальных экранов).
