...
export const MISSING_LINES = [
  ...
  'Количество ночных пробуждений и время каждого, а также время засыпания: в отчёте есть время в постели, суммарное время бодрствования за ночь и эффективность сна — и только за те ночи, где источник их измерил',
...
export function toMarkdown(model: DoctorReportModel, lang: 'ru' | 'en'): string {
  ...
  if (model.sleep) {
    ...
    p(`## ${t('Сон по дням')}`)
    p()
    p(t('Все ночи периода без агрегации. В таблице только измеренные значения: доли фаз считаются от общего сна за ночь; время, не отнесённое ни к одной фазе, показано отдельной колонкой. Время в постели и эффективность — не измерения источника, а арифметика по двум измеренным числам.'))
    p()
    table(
      [t('Дата'), t('День'), t('Отбой'), t('Подъём'), t('Сон, ч'), t('Глубокий, ч'),
        t('REM, ч'), t('Лёгкий, ч'), t('Не классифицировано, ч'),
        t('Бодрствование, мин'), t('В постели, ч'), t('Эффективность'),
