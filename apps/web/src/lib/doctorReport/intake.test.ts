...
describe('buildIntake', () => {
  it('reports only the clinically relevant types', () => {
    expect(REPORTED_TYPES).toEqual(['meds', 'alcohol', 'coffee'])
  })
  ...
  it('takes the dose median over days with a mark, not over events', () => {
    // Day one: two cups of 200 => 400. Day two: one cup of 100.
    // Median over days is 250; a median over events would be 200.
    const events = [
      ev({ ts: '2026-07-29T09:00:00', type: 'coffee', amount: 200, unit: 'мл' }),
      ev({ ts: '2026-07-29T14:00:00', type: 'coffee', amount: 200, unit: 'мл' }),
      ev({ ts: '2026-07-30T09:00:00', type: 'coffee', amount: 100, unit: 'мл' }),
    ]
    const [line] = buildIntake(events, frame)
  ...
  it('does not name anything for types other than medication', () => {
    const events = [ev({ ts: '2026-07-29T09:00:00', type: 'coffee', note: 'латте', amount: 200, unit: 'мл' })]
    const [line] = buildIntake(events, frame)
    expect(line.names).toEqual([])
  })
  ...
  it('ignores events outside the period and types the report does not carry', () => {
    const events = [
      ev({ ts: '2026-05-01T09:00:00', type: 'coffee', amount: 200, unit: 'мл' }),
  ...
  it('orders lines as medication, alcohol, coffee regardless of input order', () => {
    const events = [
      ev({ ts: '2026-07-29T09:00:00', type: 'coffee', amount: 200, unit: 'мл' }),
      ev({ ts: '2026-07-29T20:00:00', type: 'alcohol', amount: 150, unit: 'мл' }),
      ev({ ts: '2026-07-29T08:00:00', type: 'meds', note: 'Магний' }),
    ]
    expect(buildIntake(events, frame).map(l => l.type)).toEqual(['meds', 'alcohol', 'coffee'])
  })
})
